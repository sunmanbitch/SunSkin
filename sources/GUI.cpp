...
static void changeTurretSkin(const std::int32_t skinId, const std::int32_t team) noexcept
{
    ...
    const auto& playerTeam{ cheatManager.memory->localPlayer->team };

    for (const auto& turret : cheatManager.memory->turrets) {
        if (turret->team != team)
            continue;
...
{
    const auto& cheatManager{ CheatManager::getInstance() };
    std::call_once(set_font_scale, [&] { ImGui::GetIO().FontGlobalScale = cheatManager.config->fontScale; });

    const auto player{ cheatManager.memory->localPlayer };
    static const auto my_team{ player ? player->team : 100 };
    ...
    static const auto vector_getter_skin = [](void* vec, const std::int32_t idx, const char** out_text) noexcept {
        ...
        *out_text = vector[idx].skin_name.c_str();
        return true;
        };
    ...
    ImGui::Begin(LOGO, nullptr, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_AlwaysAutoResize);
    {
        ...
        if (ImGui::BeginTabBar("TabBar", ImGuiTabBarFlags_Reorderable | ImGuiTabBarFlags_FittingPolicyScroll | ImGuiTabBarFlags_NoTooltip)) {
            if (player && ImGui::BeginTabItem(PLAYER)) {
                ...
                if (ImGui::Combo(CURRSKIN, &cheatManager.config->current_combo_skin_index, vector_getter_skin, static_cast<void*>(&values), values.size()))
                    player->change_skin(values[cheatManager.config->current_combo_skin_index].model_name, values[cheatManager.config->current_combo_skin_index].skin_id);
                ...
                {
                    auto gearsVector{ std::vector<const char*>{it->second.gears.begin(), it->second.gears.end()} };
                    const auto& stack{ player->get_character_data_stack() };
                    // static int gear{ player ? player->get_character_data_stack()->base_skin.gear : 0 };

                    std::int32_t gear{ stack->base_skin.gear };

                    if (ImGui::Combo(CURRGEAR, &gear, vector_getter_gear, static_cast<void*>(&gearsVector), it->second.gears.size())) {
                        stack->base_skin.gear = static_cast<std::int8_t>(gear);
                        stack->update();
                    }
                    ImGui::Separator();
                }
            ...
            if (cheatManager.database->heroSkinIndex.size() > 1 && ImGui::BeginTabItem("Other Champs")) {
                ...
                for (const auto& hero : cheatManager.memory->heroes) {
                    ...
                    const auto hero_team{ hero->team };
                    ...
                    std::snprintf(this->str_buffer, sizeof(this->str_buffer), cheatManager.config->heroName ? "HeroName: [ %s ]##%X" : "PlayerName: [ %s ]##%X", cheatManager.config->heroName ? hero->get_character_data_stack()->base_skin.model.str : hero->name.c_str(), reinterpret_cast<std::uintptr_t>(hero));

                    auto& values{ cheatManager.database->champions_skins[champion_name_hash] };
                    if (ImGui::Combo(str_buffer, &fst->second, vector_getter_skin, static_cast<void*>(&values), values.size()))
